"""
Auto-generated from 04_silver_stream_hvfhv.ipynb
"""
import sys
...
from src.transformations.tlc_rules import (
    HVFHV_RULES, apply_quality_flags, route_quarantine
)
...
KAFKA_TOPIC     = os.getenv("KAFKA_TOPIC_HVFHV_2026",  "tlc-hvfhv-2026")
VEHICLE_TYPE    = "hvfhv"
...
spark = get_spark_streaming(
    app_name="TLC_Silver_Stream_HVFHV_2026",
...
HVFHV_RECORD_SCHEMA = T.StructType([
    ...
    T.StructField("request_datetime",       T.StringType(),  True),
    T.StructField("on_scene_datetime",      T.StringType(),  True),
    ...
    T.StructField("trip_time",              T.DoubleType(),  True),
...
def process_batch(batch_df: DataFrame, batch_id: int) -> None:
    ...
    flagged_df = apply_quality_flags(parsed_df, HVFHV_RULES)
    ...
    if records_valid > 0:
        ...
        silver_df = build_hvfhv_silver(enriched_df, run_id=stream_run_id)
        ...
        print(f"  ✓ Silver written: {n_silver:,} rows → tlc_silver.trips_hvfhv")
    ...
    if records_rejected > 0:
        ...
        quarantine_df_mapped = rejected_df.select(
            ...
            F.lit("silver_stream_hvfhv").alias("pipeline"),
    ...
    audit_record = {
        ...
        "pipeline_name":  "silver_stream_hvfhv",
        "status":         "SUCCESS",
